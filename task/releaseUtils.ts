...
export const packages = readdirSync(resolve(process.cwd(), "./packages"));
...
export async function getLatestTag(pkgName: string, version: string) {
  ...
  const prefix =
    tags.filter((tag: string) => tag.startsWith(`${pkgName}@`)).length > 0
      ? `${pkgName}@`
      : "v";
  return tags
    .filter(
      (tag: string) =>
        ...
        (isBeta ? tag.includes("beta") : !tag.includes("beta"))
    )
